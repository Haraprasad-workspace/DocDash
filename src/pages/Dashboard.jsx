    ...
    <main className='min-h-screen bg-page-bg py-12 px-4 sm:px-6 lg:px-8'>
      <div className='max-w-7xl mx-auto space-y-16'>
        ...
        <div className='flex flex-col md:flex-row justify-between items-start md:items-end gap-6'>
          <div>
            <h1 className='text-4xl font-bold tracking-tight text-brand-text-primary'>
              My Orders
            </h1>
            <p className='text-brand-text-muted mt-2 text-lg'>
              Track and manage your print jobs
            </p>
          </div>
          <button
        ...
        <section>
          <div className='flex items-center gap-3 mb-8'>
            ...
            {activeOrders.length > 0 && (
              <span className='bg-brand-surface-primary text-brand-text-primary px-3 py-1 rounded-full text-sm font-medium'>
                {activeOrders.length}
              </span>
            )}
          </div>
          ...
          {activeOrders.length === 0 ? (
            <div className='bg-brand-surface-secondary/30 rounded-3xl border border-dashed border-border-default p-16 text-center'>
              ...
              <button
                onClick={() => navigate("/user/upload")}
                className='mt-4 text-brand-text-link hover:underline'
              >
                Start a new order
              </button>
            </div>
              ...
              {activeOrders.map((order) => (
                ...
                >
                  ...
                  <p className='text-sm text-brand-text-muted mb-6'>
                    {order.totalPages} Pages • ₹{order.totalPrice}
                  </p>
        ...
        {pastOrders.length > 0 && (
          <section className='pt-8 border-t border-divider-light'>
            <h2 className='text-2xl font-bold text-brand-text-muted mb-8'>
              Past Orders
            </h2>
              ...
              {pastOrders.map((order) => (
                ...
                >
                  ...
                  <div className='flex items-center justify-between'>
                    <h3 className='text-base font-semibold text-brand-text-body'>
                      ₹{order.totalPrice}
                    </h3>
